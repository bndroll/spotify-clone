...
import { IRegisterUser } from '../store/auth/types';
import { useSelector } from 'react-redux';
import { selectUserData, selectUserErrorMessage } from '../store/user/selectors';
...
const yupSchema = yup.object({
	...
	role: yup.string().oneOf(['Пользователь', 'Музыкант'], 'Пользователь или музыкант').required('Введите роль'),
	password: yup.string().min(5, 'Пароот должен быть больше чем 5 символов').required('Введите пароль'),
...
interface IRegisterFormProps {
	...
	setAuthType: (param: 'login' | 'register') => void;
}

export const RegisterForm: React.FC<IRegisterFormProps> = ({setShowErrorMessage, setAuthType}) => {
	...
	const onSubmit = (data: IRegisterForm) => {
		const requestData: IRegisterUser = {
			name: data.name,
			email: data.email,
			password: data.password,
			role: data.role === 'Пользователь' ? 'user' : 'musician'
		};

		dispatch(singUp(requestData));
	};
			...
			<InputsWrapper>
				...
				<Password2Container>
					<Input {...register('password2')}
						   ...
						   id="password2_field"/>
					<Label htmlFor="password2_field" error={!!errors.password2}>
				...
				<RoleContainer>
					<Input {...register('role')}
						   placeholder="Введите роль (пользователь, музыкант)"
						   id="role_field"/>
					<Label htmlFor="role_field" error={!!errors.role}>
						{errors.role?.message ? errors.role?.message : `Роль`}
					</Label>
				</RoleContainer>
			</InputsWrapper>
...
const InputsWrapper = styled.div`
  ...
  grid-template-areas: "email email" "password password2" "name name" "role role";
  grid-gap: 15px;
`;
...
const Password2Container = styled(FieldContainer)`
  grid-area: password2;
`;
...
const RoleContainer = styled(FieldContainer)`
  grid-area: role;
`;
